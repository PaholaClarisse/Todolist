...
@app.get("/tache/toutes", response_model = List[Tache])
def list_tache():
    return taches

#lister les taches en utilisant la pagination
@app.get("/tache/pagination", response_model = List[Tache] )
def list_tache_pagination(debut: int = 0, limit: int = 5) -> List[Tache]:
    resultat = taches[debut: debut + limit]
    return resultat
...
@app.get("/tache/etat/{statut_tache}", response_model = List[Tache])
...
@app.get("/tache/{statut_tache}", response_model = List[Tache])
...
@app.get("/tache/date/{date_precis}", response_model = List[Tache])
...
@app.delete("/tache/id/{id_tache}", response_model = List[Tache])
...
@app.delete("/tache/titre/{titre_tache}", response_model = List[Tache])
